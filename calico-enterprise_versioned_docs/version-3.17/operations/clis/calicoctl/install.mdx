...
If you get the error, `cannot be opened because the developer cannot be verified` when using `caicoctl` for the first time, go to `Applications > System Preferences > Security & Privacy` in the `General` tab at the bottom of the window click `Allow anyway`.
